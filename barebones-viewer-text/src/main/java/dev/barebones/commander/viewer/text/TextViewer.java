...
import static dev.barebones.commander.viewer.text.TextViewerSnapshot.TEXT_FILE_PRESENTER_SECTION;
...
import dev.barebones.commander.commons.io.EncodingDetector;
import dev.barebones.commander.commons.io.RandomAccessInputStream;
import dev.barebones.commander.commons.io.bom.BOMInputStream;
...
import dev.barebones.commander.ui.encoding.EncodingMenu;
import dev.barebones.commander.viewer.FileViewer;
import dev.barebones.commander.viewer.ViewerPresenter;
...
public class TextViewer implements FileViewer, EncodingListener, ActionListener {
    ...
    void startEditing(AbstractFile file, DocumentListener documentListener) throws IOException {
        ...
        try {
            ...
            AtomicBoolean is8bit = new AtomicBoolean();
            // The FilterInputStream Wraps/Proxies a given InputStream to snoop if any read characters were
            // 8bit to distinguish ISO-8859-1 (and others) from 7-bit ASCII (US-ASCII).
            // See: https://github.com/mucommander/mucommander/issues/824
            String encoding = EncodingDetector.detectEncoding(new FilterInputStream(in) {
                @Override
                public int read() throws IOException {
                    int b = in.read();
                    if (!is8bit.get() && check8Bit(b)) {
                        is8bit.set(true);
                    }
                    return b;
                }
