...
import dev.barebones.commander.commons.file.archive.zip.provider.ZipConstants;
import dev.barebones.commander.commons.file.archive.zip.provider.ZipEntry;
import dev.barebones.commander.commons.file.archive.zip.provider.ZipFile;
 ...
 * <code>dev.barebones.commander.commons.file.impl.zip.provider</code> package and Java's <code>java.util.zip</code>.
 * <code>dev.barebones.commander.commons.file.impl.zip.provider</code> provides additional functionality and improved performance over
 ...
 * </p>
 *
 * @see dev.barebones.commander.commons.file.archive.zip.ZipFormatProvider
 * @see dev.barebones.commander.commons.file.archive.zip.provider.ZipFile
...
public class ZipArchiveFile extends AbstractRWArchiveFile {
    ...
    /**
     * Creates and returns a {@link dev.barebones.commander.commons.file.archive.zip.provider.ZipEntry} instance using the attributes
    ...
    private ZipEntry createZipEntry(ArchiveEntry entry) {
        ...
        dev.barebones.commander.commons.file.archive.zip.provider.ZipEntry zipEntry = new dev.barebones.commander.commons.file.archive.zip.provider.ZipEntry(path);
    ...
    /**
     * Creates and return an {@link ArchiveEntry()} whose attributes are fetched from the given {@link dev.barebones.commander.commons.file.archive.zip.provider.ZipEntry}.
    ...
    public synchronized InputStream getEntryInputStream(ArchiveEntry entry, ArchiveEntryIterator entryIterator) throws IOException, UnsupportedFileOperationException {
        ...
        if (file.isFileOperationSupported(FileOperation.RANDOM_READ_FILE)) {
            ...
            ZipEntry zipEntry = (dev.barebones.commander.commons.file.archive.zip.provider.ZipEntry)entry.getEntryObject();
     ...
     * read-only access and behaves just like a {@link dev.barebones.commander.commons.file.archive.AbstractROArchiveFile}.
     *
