 ...
 *
 * <p>
 * When {@link #moveToTrash(dev.barebones.commander.commons.file.AbstractFile)} is called, the file is added to a queue.
 ...
 * of moving a file to the trash has a high cost and {@link #moveToTrash(dev.barebones.commander.commons.file.AbstractFile)} is called
 * repeatedly. One thing to note is since the move is performed asynchronously,
 * {@link #moveToTrash(dev.barebones.commander.commons.file.AbstractFile)} returns immediately without waiting for the file to be moved,
...
public abstract class QueuedTrash extends AbstractTrash {
     ...
     * {@link #canMoveToTrash(dev.barebones.commander.commons.file.AbstractFile)} returned <code>true</code> for it.
     ...
     *
     * <p>The thread starts by waiting {@link dev.barebones.commander.desktop.osx.OSXTrash#QUEUE_PERIOD} milliseconds before moving them to give additional
     * files a chance to be queued and regrouped as a single call to {@link QueuedTrash#moveToTrash(java.util.List)}.
     * If more files were queued during that period, the thread will wait an additional {@link dev.barebones.commander.desktop.osx.OSXTrash# QUEUE_PERIOD},
     * and so on.<p>
     */
