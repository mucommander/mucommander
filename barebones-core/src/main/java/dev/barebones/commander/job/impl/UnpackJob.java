...
package dev.barebones.commander.job.impl;
...
import dev.barebones.commander.job.FileJobAction;
import dev.barebones.commander.job.FileJobState;
...
import dev.barebones.commander.ui.dialog.file.ArchivePasswordDialog;
 ...
 * order using {@link dev.barebones.commander.commons.file.archive.AbstractArchiveFile#getEntryIterator()}, to traverse the archive only once
...
public class UnpackJob extends AbstractCopyJob {
     ...
     * @param fileExistsAction default action to be performed when a file already exists in the destination, see {@link dev.barebones.commander.ui.dialog.file.FileCollisionDialog} for allowed values
     */
    public UnpackJob(ProgressDialog progressDialog, MainFrame mainFrame, FileSet files, AbstractFile destFolder, FileCollisionDialog.FileCollisionAction fileExistsAction) {
     ...
     */
    public UnpackJob(ProgressDialog progressDialog, MainFrame mainFrame, AbstractArchiveFile archiveFile, int baseArchiveDepth, AbstractFile destFolder, String newName, FileCollisionDialog.FileCollisionAction fileExistsAction, List<ArchiveEntry> selectedEntries) {
