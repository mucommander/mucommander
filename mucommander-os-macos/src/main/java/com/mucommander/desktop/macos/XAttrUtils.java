...
public class XAttrUtils {
    ...
    public static byte[] read(String path, String key) {
        long bufferLength = MacXAttr.INSTANCE.getxattr(path, key, null, 0, 0, 0);
        if (bufferLength <= 0)
            return null;

        Memory valueBuffer = new Memory(bufferLength);
        valueBuffer.clear();
        long valueLength = MacXAttr.INSTANCE.getxattr(path, key, valueBuffer, bufferLength, 0, 0);
    ...
    public static void write(String path, String key, byte[] value) {
        ...
        MacXAttr.INSTANCE.setxattr(path, key, valueBuffer, valueBuffer.size(), 0, 0);
    }
}
