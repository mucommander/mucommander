...
import com.mucommander.file.impl.sevenzip.provider.Common.LimitedSequentialInStream;
import com.mucommander.file.impl.sevenzip.provider.Common.LockedInStream;
import com.mucommander.file.impl.sevenzip.provider.Common.LockedSequentialInStreamImp;
...
import com.mucommander.file.impl.sevenzip.provider.SevenZip.ICompressCoder;
import com.mucommander.file.impl.sevenzip.provider.SevenZip.ICompressCoder2;
import com.mucommander.file.impl.sevenzip.provider.SevenZip.ICompressFilter;
import com.mucommander.file.impl.sevenzip.provider.SevenZip.ICompressSetDecoderProperties2;
import com.mucommander.file.impl.sevenzip.provider.SevenZip.Archive.Common.BindPair;
import com.mucommander.file.impl.sevenzip.provider.SevenZip.Archive.Common.CoderMixer2;
import com.mucommander.file.impl.sevenzip.provider.SevenZip.Archive.Common.CoderMixer2ST;
import com.mucommander.file.impl.sevenzip.provider.SevenZip.Archive.Common.CoderStreamsInfo;
import com.mucommander.file.impl.sevenzip.provider.SevenZip.Archive.Common.FilterCoder;
...
class Decoder {
    ...
    static void ConvertFolderItemInfoToBindInfo(Folder folder,BindInfoEx bindInfo) {
        ...
        for (int i = 0; i < folder.BindPairs.size(); i++) {
            ...
            bindPair.InIndex = ((BindPair) folder.BindPairs.get(i)).InIndex;
            bindPair.OutIndex = ((BindPair) folder.BindPairs.get(i)).OutIndex;
        ...
        for (int i = 0; i < folder.Coders.size(); i++) {
            ...
            AltCoderInfo altCoderInfo = (AltCoderInfo) coderInfo.AltCoders.Front();
    ...
    static boolean AreBindInfoExEqual(BindInfoEx a1, BindInfoEx a2) {
        ...
        for (i = 0; i < a1.Coders.size(); i++)
            if (!AreCodersEqual((CoderStreamsInfo) a1.Coders.get(i), (CoderStreamsInfo) a2.Coders.get(i)))
                return false;
        if (a1.BindPairs.size() != a2.BindPairs.size())
            return false;
        for (i = 0; i < a1.BindPairs.size(); i++)
            if (!AreBindPairsEqual((BindPair) a1.BindPairs.get(i), (BindPair) a2.BindPairs.get(i)))
                return false;
    ...
    int Decode(com.mucommander.file.impl.sevenzip.provider.SevenZip.IInStream inStream,
            long startPos,
            LongVector packSizes, int packSizesOffset, // const UInt64 *packSizes,
            Folder folderInfo,
            java.io.OutputStream outStream,
            com.mucommander.file.impl.sevenzip.provider.SevenZip.ICompressProgressInfo compressProgress // , // ICompressProgressInfo *compressProgress,
            // _ST_MODE boolean mtMode, int numThreads
            ) throws IOException {
        
        
        ObjectVector inStreams = new ObjectVector(); // CObjectVector< CMyComPtr<ISequentialInStream> >
            ...
            for (i = 0; i < numCoders; i++) {
                ...
                if (coderInfo.IsSimpleCoder()) {
                    ...
                    if (altCoderInfo.MethodID.equals(MethodID.k_LZMA))
                        decoder = new com.mucommander.file.impl.sevenzip.provider.SevenZip.Compression.LZMA.Decoder(); // NCompress::NLZMA::CDecoder;
                    ...
                    if (altCoderInfo.MethodID.equals(MethodID.k_BCJ_X86))
                        filter = new com.mucommander.file.impl.sevenzip.provider.SevenZip.Compression.Branch.BCJ_x86_Decoder();
                    ...
                    if (altCoderInfo.MethodID.equals(MethodID.k_Copy))
                        decoder = new com.mucommander.file.impl.sevenzip.provider.SevenZip.Compression.Copy.Decoder(); // decoder = new NCompress::CCopyCoder;
                    ...
                    if (altCoderInfo.MethodID.equals(MethodID.k_BCJ2)) {
                        decoder = new com.mucommander.file.impl.sevenzip.provider.SevenZip.Compression.Branch.BCJ2_x86_Decoder();
                    }
            ...
            for (j = 0; j < numInStreams; j++, packStreamIndex++) {
                ...
                if (bindPairIndex >= 0)
                    packSizesPointers.add(
                            folderInfo.UnPackSizes.get(((BindPair) folderInfo.BindPairs.get(bindPairIndex)).OutIndex));
                else {
        ...
        if (numCoders == 0)
            return 0;
        RecordVector inStreamPointers = new RecordVector(); // CRecordVector<ISequentialInStream *>
        inStreamPointers.Reserve(inStreams.size());
        for (i = 0; i < inStreams.size(); i++)
            inStreamPointers.add(inStreams.get(i));

        RecordVector outStreamPointer = new RecordVector();
