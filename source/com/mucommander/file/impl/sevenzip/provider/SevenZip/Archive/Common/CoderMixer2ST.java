...
import com.mucommander.file.impl.sevenzip.provider.SevenZip.ICompressSetInStream;
import com.mucommander.file.impl.sevenzip.provider.SevenZip.ICompressSetOutStream;
import com.mucommander.file.impl.sevenzip.provider.SevenZip.ICompressSetOutStreamSize;


public class CoderMixer2ST implements ICompressCoder2 , CoderMixer2 {
    
    BindInfo _bindInfo = new BindInfo();
    ObjectVector _coders = new ObjectVector();
    int _mainCoderIndex;
    ...
    public void AddCoderCommon(boolean isMain) {
        CoderStreamsInfo csi = (CoderStreamsInfo) _bindInfo.Coders.get(_coders.size());
    ...
    public void AddCoder2(ICompressCoder2 coder, boolean isMain) {
        ...
        ((STCoderInfo) _coders.Back()).Coder2 = coder;
    }
    
    public void AddCoder(ICompressCoder coder, boolean isMain) {
        AddCoderCommon(isMain);
        ((STCoderInfo) _coders.Back()).Coder = coder;
    }
    ...
    public void SetCoderInfo(int coderIndex,LongVector inSizes, LongVector outSizes) {
    	...
    	((STCoderInfo) _coders.get(coderIndex)).SetCoderInfo(inSizes, outSizes);
    }
    ...
    public int GetInStream(
        ...
        for(i = 0; i < _bindInfo.InStreams.size(); i++)
            ...
            seqInStream = (InputStream) inStreams.get(i);
        ...
        _bindInfo.FindOutStream(((BindPair) _bindInfo.BindPairs.get(binderIndex)).OutIndex,
                tmp1 /* coderIndex */ , tmp2 /* coderStreamIndex */ );
        int coderIndex = tmp1[0], coderStreamIndex = tmp2[0];
        
        CoderInfo coder = (STCoderInfo) _coders.get(coderIndex);
        if (coder.Coder == null)
    ...
    public int GetOutStream(
        ...
        for(i = 0; i < _bindInfo.OutStreams.size(); i++)
            ...
            seqOutStream = (OutputStream) outStreams.get(i);
        ...
        int tmp1[] = new int[1];
        int tmp2[] = new int[1];
        _bindInfo.FindInStream(((BindPair) _bindInfo.BindPairs.get(binderIndex)).InIndex,
                tmp1 /* coderIndex*/ , tmp2 /* coderStreamIndex */ );
        int coderIndex = tmp1[0], coderStreamIndex = tmp2[0];
    ...
    public int Code(
        ...
        for (i = 0; i < _coders.size(); i++)
            if (((STCoderInfo) _coders.get(i)).IsMain) {
            _mainCoderIndex = i;
            break;
            }
        if (_mainCoderIndex < 0)
            for (i = 0; i < _coders.size(); i++)
                if (((STCoderInfo) _coders.get(i)).NumInStreams > 1) {
            if (_mainCoderIndex >= 0)
                return HRESULT.E_NOTIMPL;
            _mainCoderIndex = i;
                }
        ...
        CoderInfo mainCoder = (STCoderInfo) _coders.get(_mainCoderIndex);
        
        ObjectVector seqInStreams = new ObjectVector(); // CObjectVector< CMyComPtr<ISequentialInStream> >
        ObjectVector seqOutStreams = new ObjectVector(); // CObjectVector< CMyComPtr<ISequentialOutStream> >
        ...
        RecordVector seqInStreamsSpec = new RecordVector();
        RecordVector seqOutStreamsSpec = new RecordVector();
        for (i = 0; i < (int)mainCoder.NumInStreams; i++)
        ...
        for (i = 0; i < _coders.size(); i++) {
            if (i == _mainCoderIndex)
                continue;
            CoderInfo coder = (STCoderInfo) _coders.get(i);
        ...
        if (mainCoder.Coder != null) {
            int res = mainCoder.Coder.Code(
                    (InputStream) seqInStreamsSpec.get(0),
                    (OutputStream) seqOutStreamsSpec.get(0),
            ...
            int res = mainCoder.Coder2.Code(
                    ...
                    Long.valueOf(mainCoder.InSizePointers.Front()), // &mainCoder.InSizePointers.Front()
                    mainCoder.NumInStreams,
                    seqOutStreamsSpec, // &seqOutStreamsSpec.Front()
                    Long.valueOf(mainCoder.OutSizePointers.Front()), // &mainCoder.OutSizePointers.Front()
        ...
        java.io.OutputStream stream = (OutputStream) seqOutStreams.Front();
        stream.flush();
