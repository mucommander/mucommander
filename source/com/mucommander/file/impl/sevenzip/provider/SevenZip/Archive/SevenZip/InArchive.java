...
import com.mucommander.file.impl.sevenzip.provider.Common.CRC;
...
import com.mucommander.file.impl.sevenzip.provider.SevenZip.Common.StreamUtils;
...
class InArchive extends Header {
    ...
    public void AddByteStream(byte [] buffer, int size) {
        ...
        _inByteBack = (InByte2) _inByteVector.Back();
    ...
    int GetNextFolderItem(Folder folder)  throws IOException {
        ...
        for (i = 0; i < numCoders; i++) {
            ...
            CoderInfo coder = (CoderInfo) folder.Coders.Back();
            
            for (;;) {
                coder.AltCoders.add(new AltCoderInfo());
                AltCoderInfo altCoder = (AltCoderInfo) coder.AltCoders.Back();
    ...
    int ReadFileNames(ObjectVector files)  throws IOException {
        for(int i = 0; i < files.size(); i++) {
            ...
            ((FileItem) files.get(i)).name = name;
        }
        return HRESULT.S_OK;
    }
    ...
    int ReadUnPackInfo(
            ObjectVector dataVector,
            ObjectVector folders)  throws IOException {
        ...
        
        {
            ...
            for(int i = 0; i < numFolders; i++) {
                ...
                ret = GetNextFolderItem((Folder) folders.Back());
                if (ret != HRESULT.S_OK) {
                    streamSwitch.close();
                    return ret;
                }
    ...
    int ReadTime(ObjectVector dataVector,
            ObjectVector files, long type)  throws IOException {
    ...
    int ReadAndDecodePackedStreams(long baseOffset,
            long [] dataOffset,
            ObjectVector dataVector // CObjectVector<CByteBuffer> &dataVector
            ) throws IOException {
        ...
        ObjectVector folders = new ObjectVector();
        ...
        for(int i = 0; i < folders.size(); i++) {
            Folder folder = (Folder) folders.get(i); // const CFolder &folder = folders[i];
            dataVector.add(new ByteBuffer());
            ByteBuffer data = (ByteBuffer) dataVector.Back();
            ...
            com.mucommander.file.impl.sevenzip.provider.SevenZip.Common.SequentialOutStreamImp2 outStreamSpec = new com.mucommander.file.impl.sevenzip.provider.SevenZip.Common.SequentialOutStreamImp2();
 ...
 */
        ...
        ObjectVector dataVector = new ObjectVector(); // CObjectVector<CByteBuffer> dataVector;
        ...
        for (;;) {
            ...
            if (dataVector.size() > 1)
                throw new IOException("Incorrect Header"); // CInArchiveException(CInArchiveException::kIncorrectHeader);
            streamSwitch.Remove();
            streamSwitch.Set(this, (ByteBuffer) dataVector.get(0)); // dataVector.Front()
        }
    ...
    int ReadHeader(ArchiveDatabaseEx database)  throws IOException {
        ...
        ObjectVector dataVector = new ObjectVector();
