...
public class BufferPoolTest extends TestCase {
    ...
    public void testBuffer(BufferPool.BufferFactory factory) {
        ...
        assertFalse(BufferPool.containsBuffer(buffer1, factory));
        assertFalse(BufferPool.containsBuffer(buffer2, factory));
        assertFalse(BufferPool.containsBuffer(buffer3, factory));
        ...
        assertTrue(BufferPool.containsBuffer(buffer3, factory));
        assertTrue(buffer3==BufferPool.getBuffer(factory, TEST_BUFFER_SIZE_2));
        assertBufferCount(originalBufferCount, factory);
        assertFalse(BufferPool.containsBuffer(buffer3, factory));
        ...
        for(int b=0; b<buffers.length; b++) {
            ...
            assertTrue(BufferPool.releaseBuffer(buffers[b], factory));
            assertTrue(BufferPool.containsBuffer(buffers[b], factory));
            assertBufferCount(originalBufferCount+(b+1), factory);

            assertFalse(BufferPool.releaseBuffer(buffers[b], factory));
            assertTrue(BufferPool.containsBuffer(buffers[b], factory));
            assertBufferCount(originalBufferCount+(b+1), factory);
        }
        ...
        assertEquals(BufferPool.INITIAL_DEFAULT_BUFFER_SIZE, BufferPool.getDefaultBufferSize());
        ...
        assertEquals(TEST_BUFFER_SIZE_1, BufferPool.getDefaultBufferSize());
        ...
        assertEquals(BufferPool.INITIAL_POOL_LIMIT, BufferPool.getMaxPoolSize());
        BufferPool.setMaxPoolSize(TEST_MAX_POOL_SIZE);
        assertEquals(TEST_MAX_POOL_SIZE, BufferPool.getMaxPoolSize());
        ...
        long newPoolSize;
        for(int i=0; i<nbBuffers; i++) {
            assertTrue(BufferPool.releaseBuffer(buffers[i], factory));
            newPoolSize = BufferPool.getPoolSize();
            assertEquals(lastPoolSize+bufferSize, newPoolSize);
        ...
        assertFalse(BufferPool.releaseBuffer(extraBuffer, factory));
    ...
    private void assertBufferSize(Object buffer, BufferPool.BufferFactory factory, int expectedSize) {
        assertEquals(expectedSize, getBufferLength(buffer, factory));
    }

    /**
     * Returns the length of the given buffer, as defined by {@link com.mucommander.io.BufferPool.BufferContainer#getLength()}.
     *
    ...
    /**
     * Returns the size in bytes of the given buffer, as defined by {@link com.mucommander.io.BufferPool.BufferContainer#getSize()}.
     *
