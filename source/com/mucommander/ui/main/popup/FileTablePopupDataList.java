...
class FileTablePopupDataList extends JList implements FileTablePopupFocusableComponent {	
	private final static int VISIBLE_ROWS_COUNT = 10;
	private boolean isDataUpToDate = true;
	private Object[] updatedData;
	
	public FileTablePopupDataList(){
	 ...
	 * 	should be shown.
	 */
	public boolean prepareToBeShown() {
		// if the up-to-date data is not set, then set it. 
		if (!isDataUpToDate) {
			super.setListData(updatedData);
		...
		// there are no elements in the list, return false to show TablePopupWithEmptyMsg.
		return false;
	}
		...
		addKeyListener(new KeyListener() {
			...
			public void keyPressed(KeyEvent e) {			
				...
				case KeyEvent.VK_ENTER:
					((FileTablePopupWithDataList)(getParent().getParent().getParent())).itemSelected((String) getSelectedValue());
					break;
				case KeyEvent.VK_UP:
					{
	...
	public FileTable getInvokerFileTable() {
		return ((FileTablePopupWithDataList)(getParent().getParent().getParent())).getPanel().getFileTable(); 
	}	
}
