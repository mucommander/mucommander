 ...
 * "@":  "&#064;" 
 */
package com.mucommander.commons.file.impl.rar.provider.de.innosystec.unrar.unpack.vm;

import com.mucommander.commons.file.impl.rar.provider.de.innosystec.unrar.crc.RarCRC;
...
//		mem[offset + 3] = (byte) ((value >>> 24)&0xff);
	...
	public void execute(VMPreparedProgram prg) {
		...
		if (dataSize != 0) {
			...
			{
				prg.getGlobalData().set(i, Byte.valueOf(mem[VM_GLOBALMEMADDR + i]));
			}
		}
	}
	...
	private boolean ExecuteCode(List preparedCode, int cmdCount) {
		...
		while (true) {
			...
			case VMCommands.VM_JZ_CODE:
				if ((flags & VMFlags.VM_FZ.getFlag()) != 0) {
					setIP(getValue(false, mem, op1));
					continue;
				}
				break;
			case VMCommands.VM_JNZ_CODE:
				if ((flags & VMFlags.VM_FZ.getFlag()) == 0) {
					setIP(getValue(false, mem, op1));
					continue;
				}
				break;
			case VMCommands.VM_INC_CODE: {
			...
			case VMCommands.VM_INCB_CODE:
				...
				break;
			case VMCommands.VM_INCD_CODE:
				setValue(false, mem, op1, (int) ((long) getValue(false, mem,
						op1) & 0xFFffFFff + 1));
				break;

			case VMCommands.VM_DEC_CODE: {
				int result = (int) ((long) getValue(cmd.isByteMode(), mem, op1) & 0xFFffFFff - 1);
				setValue(cmd.isByteMode(), mem, op1, result);
				flags = result == 0 ? VMFlags.VM_FZ.getFlag() : result
						& VMFlags.VM_FS.getFlag();
			...
			case VMCommands.VM_DECB_CODE:
				setValue(
						...
						(int) ((long) getValue(true, mem, op1) & 0xFFffFFff - 1));
				break;
			case VMCommands.VM_DECD_CODE:
				setValue(false, mem, op1, (int) ((long) getValue(false, mem,
						op1) & 0xFFffFFff - 1));
				break;

			case VMCommands.VM_JMP_CODE:
				setIP(getValue(false, mem, op1));
				continue;
			case VMCommands.VM_XOR_CODE: {
			...
			case VMCommands.VM_STANDARD_CODE:
				ExecuteStandardFilter(VMStandardFilters.findFilter(cmd.getOp1()
						.getData()));
				break;
			case VMCommands.VM_PRINT_CODE:
				break;
			}
			IP++;
			--maxOpCount;
	...
	public void prepare(byte[] code, int codeSize, VMPreparedProgram prg) {
		...
		if (xorSum == code[0]) {
			...
			if (filterType != VMStandardFilters.VMSF_NONE) {
				...
				curCmd.getOp1().setData(filterType.getFilterType());
	...
	private void ExecuteStandardFilter(VMStandardFilters filterType) {
		switch(filterType.getFilterType())
		  {
		    case VMStandardFilters.VMSF_E8_CODE:
		    case VMStandardFilters.VMSF_E8E9_CODE:
		      {
		        int dataSize=R[4];
...
//		#endif
		      ...
		      break;
		    case VMStandardFilters.VMSF_ITANIUM_CODE:
		      {
		      ...
		      break;
		    case VMStandardFilters.VMSF_DELTA_CODE:
		      {
...
//		 so we need to place them back to their interleaving positions
		      ...
		      break;
		    case VMStandardFilters.VMSF_RGB_CODE:
		      {
		      ...
		      break;
		    case VMStandardFilters.VMSF_AUDIO_CODE:
		      {
		      ...
		      break;
		    case VMStandardFilters.VMSF_UPCASE_CODE:
		      {
